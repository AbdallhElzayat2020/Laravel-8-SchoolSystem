                                    ...
                                    <tbody>
                                        @foreach ($subjects as $key => $subject)
                                            @php
                                                ...
                                                $name = json_decode($subject->name, true);

                                                // Fetch names safely
                                                $name_ar = $name['ar'] ?? 'غير متوفر';
                                                $name_en = $name['en'] ?? 'Not Available';
                                            @endphp
                                            <tr>
                                                <td>{{ $key + 1 }}</td>
                                                <!-- عرض الاسم بناءً على اللغة الحالية -->
                                                <td>{{ LaravelLocalization::getCurrentLocale() == 'ar' ? $name_ar : $name_en }}
                                                </td>
                                                        ...
                                                        <div class="modal-content">
                                                            ...
                                                            <div class="modal-footer">
                                                                ...
                                                                <button type="submit" class="btn btn-danger">
                                                                    {{ trans('My_Classes_trans.submit') }}</button>
                                                            </div>
                                                        </div>
                                                    </form>
                                                </div>
